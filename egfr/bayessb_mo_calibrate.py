...
def likelihood(mcmc, position):
    ...
    akt = np.sum((exp_data[:,0] - ysim_slice[:,0]) ** 2 / (2 * exp_data_var[:,0] ** 2))
    erb1 = np.sum((exp_data[:,1] - ysim_slice[:,1]) ** 2 / (2 * exp_data_var[:,1] ** 2))
    erk = np.sum((exp_data[:,2] - ysim_slice[:,2]) ** 2 / (2 * exp_data_var[:,2] ** 2))
    try:
        objAKT[mcmc.iter] = akt
        objErb[mcmc.iter] = erb1
        objERK[mcmc.iter] = erk
        total_obj[mcmc.iter] = akt + erb1 + erk
        return np.array([akt, erb1, erk])
...
def step(mcmc):
    ...
    if mcmc.iter % 1 == 0:
        print 'iter=%-5d  sigma=%-.3f  T=%-.3f  acc=%-.3f, lkl=%s  prior=%s  post=%s' % \

...
def step(mcmc):
    ...
    if mcmc.iter % 500 == 0:
        ...
        np.savetxt(str(mcmc.iter)+output_file_prefix+'totalobj.txt', total_obj)
        np.savetxt(str(mcmc.iter)+output_file_prefix+'sobj_erk.txt', objERK)
        np.savetxt(str(mcmc.iter)+output_file_prefix+'sobj_akt.txt', objAKT)
        np.savetxt(str(mcmc.iter)+output_file_prefix+'sobj_erb.txt', objErb)
